...
export function setHasRequestedCurrentLocation(value: boolean) {
  const localStorageState = getLocalStorageState();
  localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ ...localStorageState, hasRequestedCurrentLocation: value }));
}
